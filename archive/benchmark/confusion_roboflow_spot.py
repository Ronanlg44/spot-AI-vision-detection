...
# Modèle 1 : Entraîné sur Spot uniquement
rf = Roboflow(api_key=CLE_API)
project = rf.workspace("berkay-kilic-dpfzs").project("spot-8feoa")
...
def dessiner_matrice(TP, FP, FN, nom_fichier, titre):
    ...
    sns.heatmap(matrice, annot=True, fmt="d", cmap="Greens", annot_kws={"size": 16}, 
...
print(f"{'='*40}\n📊 ÉVALUATION : MODÈLE SPÉCIALISTE (spot-8feoa)\n{'='*40}")
...
for i, nom_img in enumerate(fichiers_images):
    ...
    # Requête API avec un seuil à 10%
    result = model.predict(chemin_img, confidence=10, overlap=50).json()

    # Sauvegarde l'image avec les boîtes dessinées par l'API dans un dossier de résultats
    model.predict(chemin_img, confidence=10, overlap=50).save(os.path.join("runs/detect/resultats_benchmark/api_spot_8feoa", nom_img))
...
dessiner_matrice(TP, FP, FN, "Confusion_Matrix_API_Spot.png", "API - Spot Specialist")
